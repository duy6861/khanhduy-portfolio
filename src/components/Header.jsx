...
const Header = ({ avatar, name, contact, isEnglish }) => {
  ...
  // Hàm scroll đến section (tính cả header)
  ...
  return (
    <header className="w-full">
      {/* Navigation Bar */}
      <nav className="fixed top-0 left-0 right-0 h-20 bg-black/70 backdrop-blur-xl z-50 shadow-lg border-b border-gray-700/50 transition-all duration-300 flex items-center">
            ...
            <div className="md:hidden mt-2 space-y-1">
              {sections.map((item) => (
                <button
                  key={item.id}
                  onClick={() => scrollToSection(item.id)}
                  className={`block w-full text-left px-3 py-2 rounded-md text-sm capitalize transition-all duration-300
                    ${activeSection === item.id
      ...
      <div id="top" className="flex flex-col items-center mb-10 pt-28">
        ...
        <div className="flex flex-col sm:flex-row justify-center items-center gap-4 sm:gap-8 text-gray-300 mb-6 text-sm sm:text-base text-center sm:text-left">
          <a href={`tel:${contact.phone}`} className="flex items-center gap-2 hover:text-blue-400 transition-all duration-300">
          ...
          <a href={`mailto:${contact.email}`} className="flex items-center gap-2 hover:text-blue-400 transition-all duration-300">
          ...
          <a href={contact.website} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 hover:text-blue-400 transition-all duration-300">
            <svg className="w-5 h-5 opacity-90" fill="currentColor" viewBox="0 0 24 24">
          ...
          <span className="flex items-center gap-2 max-w-xs text-center sm:text-left">
            <svg className="w-5 h-5 opacity-70" fill="currentColor" viewBox="0 0 20 20">
